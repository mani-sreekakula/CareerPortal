...
public class ApplyJobDAOImpl extends HibernateDaoSupport implements ApplyJobDAO {
    ...
    public List<JobRoles> fetchjobrole(String JobRole) {
        List<JobRoles> jobRoles = getHibernateTemplate().find("from JobRoles j where j.jobname='" + JobRole+"'");
        return jobRoles;
    }
    ...
    public JobRoles fetchalluserdetails(String id1) {
        ...
        jobRoles = getHibernateTemplate().find("from JobRoles j where j.empid='" + id1 + "'");
        if (jobRoles.isEmpty()) {
            return null;
        } else {
