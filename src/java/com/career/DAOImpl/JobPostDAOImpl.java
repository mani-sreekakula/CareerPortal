...
public class JobPostDAOImpl extends HibernateDaoSupport implements JobPostDAO {
    ...
    public List<JobPost> fetchJobs() {
        List<JobPost> jobposts = getHibernateTemplate().find("from JobPost");
        return jobposts;
    }
    ...
    public List<JobPost> searchByJobRole(String jobRole) {
        List<JobPost> jobposts = getHibernateTemplate().find("from JobPost j where j.jobRole='"+jobRole+"'" );
        return jobposts;
    }

    @Override
    public List<JobPost> searchByLocation(String jobLocation) {
        List<JobPost> jobposts = getHibernateTemplate().find("from JobPost j where j.jobLocation='"+jobLocation+"'" );
        return jobposts;
    }
